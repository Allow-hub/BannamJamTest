...
namespace Jam::Presentation::Scenes
{
	...
	public:
			...
			: IScene{ init }
		{
			...
			// === テクスチャ読み込み (Resource使用時は直接フルパスを指定) ===
			// ⚠️ リソースファイル(.rc)に登録されているパスと完全一致させる必要があります
			...
			// === エラーチェック ===
			if (not m_background) { Print(U"Error: GAME_OVER_screen.png not loaded!"); }
			if (not m_titleGameOver) { Print(U"Error: Result_Game_Over.png not loaded!"); }
			if (not m_titleClear) { Print(U"Error: Result_Crear.png not loaded!"); }
			if (not m_titleMemory) { Print(U"Error: result_title-MEMORY.png not loaded!"); }
			if (not m_titleTime) { Print(U"Error: result_TIME.png not loaded!"); }
			if (not m_titleKill) { Print(U"Error: result_KILL.png not loaded!"); }
			if (not m_stageResultTexture) { Print(U"Error: Stage result texture not loaded: ", resultImageName); }
		}
