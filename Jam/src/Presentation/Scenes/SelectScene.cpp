...
namespace Jam::Presentation::Scenes
{
	...
	// --- ワールド選択のロジック ---
	void SelectScene::updateWorldSelect()
	{
		// --- 画面サイズに基づいた相対的な値 ---
	...
	// --- ワールド選択の描画 ---
	void SelectScene::drawWorldSelect() const
	{
		const double baseWidth = 1920.0;
		const double baseHeight = 1080.0;
	...
	// --- ステージ選択のロジック ---
	void SelectScene::updateStageSelect()
	{
		const double baseWidth = 1920.0;
		const double baseHeight = 1080.0;
	...
	// --- ステージ選択の描画 ---
	void SelectScene::drawStageSelect() const
	{
		const double baseWidth = 1920.0;
		const double baseHeight = 1080.0;
	...
	// --- (共通) ボタン描画ヘルパー関数 ---
	void SelectScene::drawButton(const RectF& rect, const String& text, bool mouseOver, const s3d::Texture& buttonTexture, const String& fontName) const
	{
