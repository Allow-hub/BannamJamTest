...
namespace Jam::Presentation::Background {
    ...
    void BackgroundRenderer::setBackgroundObjects(const Array<Jam::Domain::Background::BackgroundObject>& objects) {
        m_backgroundObjects = objects;
        generateBackgroundInstances();
        m_isLoaded = true;
    }

    void BackgroundRenderer::drawLayer(Jam::Domain::Background::ParallaxLayer layer, const Vec2& cameraOffset) const {
    ...
    bool BackgroundRenderer::isLoaded() const {
        return m_isLoaded;
    }

    size_t BackgroundRenderer::getObjectCount() const {
        return m_backgroundObjects.size();
    }

    void BackgroundRenderer::generateBackgroundInstances() {
