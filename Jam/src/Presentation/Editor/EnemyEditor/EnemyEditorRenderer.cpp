...
namespace Jam::Presentation::Editor
{
    ...
    void EnemyEditorRenderer::draw() const
    {
    ...
    void EnemyEditorRenderer::drawGUIPanel() const
    {
        ...
        m_font(U"[Enemy Editor]").draw(panelX + 10, y, Palette::White);
        y += 40;
        
        if (SimpleGUI::Button(U"Switch to Stage Editor", Vec2{panelX + 10, y}, 280))
        {
            m_switchToStageEditor = true;
        }
        y += 45;
        
        SimpleGUI::Headline(U"Enemy Type", Vec2{panelX + 10, y});
        y += 40;
        ...
        if (!isBoss)
        {
            SimpleGUI::Headline(U"Patrol", Vec2{panelX + 10, y});
            y += 40;
            
            double patrolDistance = m_service->getPatrolDistance();
            if (SimpleGUI::Slider(U"Distance: {:.0f}"_fmt(patrolDistance), patrolDistance, 50.0, 500.0, Vec2{panelX + 10, y}, 120, 120))
            {
            ...
            if (SimpleGUI::Slider(U"Wait: {:.1f}s"_fmt(waitTime), waitTime, 0.5, 5.0, Vec2{panelX + 10, y}, 120, 120))
            {
            ...
            if (SimpleGUI::Slider(U"Found: {:.0f}"_fmt(foundDistance), foundDistance, 300.0, 1000.0, Vec2{panelX + 10, y}, 120, 120))
            {
            ...
            if (hasChaseAI)
            {
                SimpleGUI::Headline(U"Chase", Vec2{panelX + 10, y});
                y += 40;
                
                double attackRange = m_service->getAttackRange();
                if (SimpleGUI::Slider(U"Attack: {:.0f}"_fmt(attackRange), attackRange, 300.0, 1000.0, Vec2{panelX + 10, y}, 120, 120))
                {
                ...
                if (SimpleGUI::Slider(U"Lose: {:.0f}"_fmt(loseRange), loseRange, 500.0, 1500.0, Vec2{panelX + 10, y}, 120, 120))
                {
                ...
                if (SimpleGUI::Slider(U"Speed: {:.1f}"_fmt(speedFactor), speedFactor, 1.0, 2.0, Vec2{panelX + 10, y}, 120, 120))
                {
        ...
        SimpleGUI::Headline(U"Control", Vec2{panelX + 10, y});
        y += 40;
        m_smallFont(U"Click: Place Enemy").draw(panelX + 10, y, Palette::White);
        y += 25;
    }
    ...
    String EnemyEditorRenderer::getEnemyTypeName(Domain::Editor::EnemyType type) const
    {
        switch (type)
        {
        case Domain::Editor::EnemyType::LittleDevil: return U"Little Devil";
        case Domain::Editor::EnemyType::Spider: return U"Spider";
        case Domain::Editor::EnemyType::Ribbon: return U"Ribbon";
        case Domain::Editor::EnemyType::Eye: return U"Eye";
        case Domain::Editor::EnemyType::Clown: return U"Clown";
        case Domain::Editor::EnemyType::Boss1_3: return U"Boss 1-3";
